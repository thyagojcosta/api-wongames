...
async function createManyToManyData(products) {
    ...
    products.forEach((product) => {
        const { developer, publisher, genres } = product;

        genres.forEach((item) => {
            categories.add(item);
        });

        // Object.keys(worksOn).forEach((item) => {
        //     platform.add(item);
        // });
    ...
    const createCall = (set, entityName) => Array.from(set)
        .map((name) => {
...
async function createGames(products) {
    await Promise.all(
        products.map(async (product) => {
            const platform = product.worksOn
            ...
            if (!item) {
                ...
                const game = await strapi.services.game.create({
                    ...
                    // platforms: getByName('Windows', "platform"),
